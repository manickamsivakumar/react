...
class Mis_model extends CI_Model
{
    ...
    public function get_all_module_values($fromdate, $todate)
    {
        ...
        foreach ($data as $key => $value) {
          ...
          $billingpos[$adddate]=$value['totelamount'];
            ...
            $alertandauth= $this->alertandauth($propid,$dbname,$cmpid);
    ...
    GROUP BY
        ...
        $totelrooms = "select count(roomid)as totel  from " . $dbname . ".room_master where activestatus !=0";
    ...
    function alertandauth($propid,$dbname,$cmpid) {
    ...
    ORDER BY
        ...
        $query = $this->db->query($sql);
        $isexists = $query->result_array();

        //echo '<pre>';
        //print_r($isexists);
        //echo '</pre>';

        $isexists = count($isexists);
        ...
        if($isexists!=2){
          ...
          $sql = 'SELECT
           ...
           // $userid = $data['userid'];
            ...
            $authtabs = ["pending", "accepted", "denied"];

            foreach($authtabs as $authtab){
                $athresult[$authtab] = [];
            }

            foreach($authmainresult as $values) {
                $statusmode = strtolower($values["authstatustype"]);
                $authstatus = $values['authstatus'];
                $authmode = strtolower($values['authmode']);
                $verifiercheck = 1;
                if($authmode == "verifiyer") {
                    $authreqtrnid = $values['authreqtrnid'];
                    $transresult = $authtransresult[$authreqtrnid];
                    $verifiervals = $transresult['verifiyer'];
                    if (count($verifiervals) > 1 && $statusmode == "pending") {
                        $otherverifyvals = [];
                        foreach($verifiervals as $verifierval) {
                            $akeys = array_keys($otherverifyvals);
                            if(!in_array(strtolower($verifierval['authstatustype']),$akeys)) {
                                $otherverifyvals[strtolower($verifierval['authstatustype'])] = [];
                            }
                            array_push($otherverifyvals[strtolower($verifierval['authstatustype'])],$verifierval);
                        }
                        $oakeys = array_keys($otherverifyvals);
                        if(in_array("approved",$oakeys)){
                            $statusmode = "approved";
                        }
                        else if(in_array("denied",$oakeys)) {
                            $statusmode = "denied";
                        }
                    }
                }
                else if($authmode == "authuser") {

                    $authreqtrnid = $values['authreqtrnid'];
                    $transresult = $authtransresult[$authreqtrnid];
                    $verifiervals = $transresult['verifiyer'];
                    if (count($verifiervals)) {
                        $authstatustypes = [];
                        foreach($verifiervals as $verifierval){
                            array_push($authstatustypes,strtolower($verifierval['authstatustype']));
                        }
                        if(in_array("approved",$authstatustypes)){
                            $verifiercheck = 0;
                        }
                    }
                    if ($verifiercheck == 1) {
                        $authstatustype = strtolower($values['authstatustype']);
                        $authstatus = $values['authstatus'];
                        $authstatusconnect[0] = "pending";
                        $authstatusconnect[1] = "denied";
                        $authstatusconnect[2] = "approved";
                        $authstatusname = $authstatusconnect[$authstatus];
                        if ($authstatusname != $authstatustype) {
                            $statusmode = $authstatusname;
                        } else if (
                            $authstatustype == "pending" &&
                            $authstatusname == $authstatustype
                        ) {
                ...
                array_push($athresult[$statusmode],$values);
            }
            ...
            if (count($athresult["pending"])) {

                //console.log(authtransresult);
                $result = $athresult["pending"];
                foreach($result as $values){
                    $authreqtrnid = $values['authreqtrnid'];
                    //var verifiercheck = 1;
                    $authmode = $values['authmode'];
